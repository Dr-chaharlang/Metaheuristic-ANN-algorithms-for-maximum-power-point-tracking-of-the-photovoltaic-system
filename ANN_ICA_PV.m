...
% Define objective function for Imperialist Competitive Algorithm
fitfunc = @(x) mse(outputs, sim(setwb(net, x), inputs));

% Initial values for Imperialist Competitive Algorithm
n_countries = 20;
n_generations = 100;

% Run Imperialist Competitive Algorithm
optimal_weights = ica(fitfunc,numel(initialWeights),-1,1,n_countries,n_generations);
